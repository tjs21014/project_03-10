...
# ╔═╡ 55ac48f8-d184-4e30-a41a-e57252779d92
using LinearAlgebra

# ╔═╡ 2dc92583-9e93-4ec1-a242-4a95173c0e1c
using Symbolics

# ╔═╡ 7699dfcb-7d45-4bf2-9050-50d9ed274f5c
using DifferentialEquations

# ╔═╡ 83e8af00-2f06-4f90-941f-6e514f832fa7
using Plots

# ╔═╡ d68993ef-4c57-45b9-93a6-8bfc31439cc7
begin
	const gravity_accel = 9.81
	const spring_stiffness = 40.0
	
	const block_mass = 1.2
	const bar_mass = 0.8
...
# ╔═╡ d1782bfd-73e6-486e-95d9-120f393bfd9b
function build_mass_matrix()
    inertia_bar = bar_mass * bar_length^2 / 12
    Diagonal([
        block_mass,
        block_mass,
        bar_mass,
...
# ╔═╡ 82fa7b12-bb54-4d53-ae27-c10e7b9cd711
@variables q[1:5]

# ╔═╡ 6d94b022-0ab5-4269-8526-06ffc24a3ac8
function constraint_equations(q)
    block_y = q[IDX_BLOCK_Y]
    bar_x   = q[IDX_BAR_X]
    bar_y   = q[IDX_BAR_Y]
    theta   = q[IDX_BAR_TH]
    block_x = q[IDX_BLOCK_X]

    pin_x = bar_x - (block_x + (bar_length/2)*cos(theta))
    pin_y = bar_y - (block_y + (bar_length/2)*sin(theta))
...
# ╔═╡ 11b60d73-692c-4068-b414-3d83aa075a19
begin
	constraint_vec = constraint_equations(q)
	constraint_jacobian = Symbolics.jacobian(constraint_vec, q)
	constraint_jacobian_fn = eval(Symbolics.build_function(constraint_jacobian, q)[1])
end

# ╔═╡ 4c45e30b-6534-431b-b3a1-5e1963a87826
function generalized_forces(state)
    block_x = state[IDX_BLOCK_X]

    [
        -spring_stiffness * block_x;
        -block_mass * gravity_accel;
        0.0;
        -bar_mass * gravity_accel;
        0.0
    ]
end

# ╔═╡ 0f737b78-94c2-4782-897c-979f3673130e
function augmented_acceleration(state)
    mass_matrix = build_mass_matrix()
    jacobian = constraint_jacobian_fn(state)
    ...
    rhs = vcat(generalized_forces(state), zeros(3))

    solution = augmented_matrix \ rhs
    accelerations = solution[1:5]
    multipliers = solution[6:end]

    accelerations, multipliers
end

# ╔═╡ 309e2e81-321d-4c99-9680-f4ae86f5f78a
function constrained_dynamics!(dstate, state, _, _)
    positions = state[1:5]
    velocities = state[6:10]

    accelerations, _ = augmented_acceleration(positions)

    dstate[1:5] .= velocities
    dstate[6:10] .= accelerations
end

# ╔═╡ 6fcbd633-b354-4def-ab8b-ebe52fcbaf26
begin
	initial_positions = [0.15, 0.0, 0.15, -0.3, pi/6]
	initial_velocities = zeros(5)
	initial_state = vcat(initial_positions, initial_velocities)
	
	time_span = (0.0, 6.0)
	
	problem = ODEProblem(constrained_dynamics!, initial_state, time_span)
	solution = solve(problem, Tsit5(), saveat=0.01)
end

# ╔═╡ eedb1a64-54cf-41a7-a7f2-f3fcbd565874
function bar_endpoints(block_x, theta)
    x_end = block_x + bar_length * cos(theta)
    y_end = bar_length * sin(theta)
    x_end, y_end
end

# ╔═╡ dfc1984c-dd2e-4727-8b9b-5a36696c0de9
begin
	animation = @animate for k in 1:length(solution.t)
	    state = solution.u[k]
	
	    bx = state[IDX_BLOCK_X]
	    θ  = state[IDX_BAR_TH]
	
	    bar_x, bar_y = bar_endpoints(bx, θ)
	
	    plot([-1, 1], [0, 0], lw=2, legend=false, aspect_ratio=1)
	    scatter!([bx], [0], ms=8)
	    plot!([bx, bar_x], [0, bar_y], lw=3)
	end
	
	gif(animation, "constrained_multibody_motion.gif", fps=30)
end

# ╔═╡ f894e2e2-75d2-4a80-8281-2a0a08a638dc
constraint_forces = [
    augmented_acceleration(solution.u[k][1:5])[2]
    for k in 1:length(solution.t)
]

# ╔═╡ efab2a84-7125-4ba1-8b8b-81a6cc77c7ea
normal_force = [λ[1] for λ in constraint_forces]

# ╔═╡ 88542cae-6a2c-4789-8669-c7ed2c777977
pin_force_x  = [λ[2] for λ in constraint_forces]

# ╔═╡ 4361b8f3-df42-44a9-a232-ca0948a75006
pin_force_y  = [λ[3] for λ in constraint_forces]

# ╔═╡ 1741fa65-4b28-4f69-b7a2-6c285806a62b
pin_resultant = sqrt.(pin_force_x.^2 .+ pin_force_y.^2)

# ╔═╡ 9ece03b7-ffc8-42ea-8755-e74546237899
plot(solution.t, pin_resultant,
     xlabel="Time (s)",
     ylabel="Resultant Pin Force (N)",
     lw=2)
...
manifest_format = "2.0"
project_hash = "277b9225145e05f3410d5fb1ae503a7168d99b66"
...
git-tree-sha1 = "dc96b003f64b75f87f9cde208c1e5e3234fc241d"
...
git-tree-sha1 = "8462dcd4ad231e8b88fb3e4781e21be4c0dad4e6"
